...
```{r}
if (good_to_go) {
  read_plink_log_file(stringr::str_subset(miss_stat_filenames, "miss_stat.log"))
}
```
...
```{r}
if (good_to_go) {
  lmiss_table <- read_plink_lmiss_file(
    stringr::str_subset(miss_stat_filenames, "miss_stat.lmiss")
  )
  knitr::kable(head(lmiss_table))
}
```
we see

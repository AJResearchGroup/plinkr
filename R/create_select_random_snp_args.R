...
create_select_random_snp_args <- function(
  ...
  plinkr::check_random_snps_selector(random_snps_selector)
  ...
  testthat::expect_true(random_snps_selector$n_snps > 0)
  ...
  if (n_snps_present < random_snps_selector$n_snps) {
    stop(
      ...
      "Number of random SNPs to pick: ", random_snps_selector$n_snps
    )
  }
  snp_indices <- sort(
    sample(
      seq(1, n_snps_present),
      size = random_snps_selector$n_snps,
      replace = FALSE
    )
  )

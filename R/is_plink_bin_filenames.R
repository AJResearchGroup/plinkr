#' Determine if the `plink_bin_files` is indeed a set of `PLINK`
#' binary files' names
#'
#' Determine if the `plink_bin_files` is indeed a set of `PLINK`
#' binary files' names
...
#' @return \link{TRUE} if the `plink_bin_files` is indeed a set of `PLINK`
#' binary files' names
...
is_plink_bin_filenames <- function(
  ...
  result <- FALSE
  tryCatch({
    plinkr::check_plink_bin_filenames(plink_bin_filenames = plink_bin_filenames)
    result <- TRUE
  }, error = function(e) {

...
Note that the `.ped` file contains a column called `phenotype_value`.
In this example, this column will be ignored, as we supply our own phenotypic
values in the phenotype file. If we do not include the file with our
own phenotypic values, PLINK will use the ones in the `.ped` file 
instead (see also below, in the section 'Is the phenotype file mandatory?')
...
However, the input is now in `run1.assoc`

...
#'     bed_filename = get_plinkr_filename("select_samples.bed"),
#'     bim_filename = get_plinkr_filename("select_samples.bim"),
#'     fam_filename = get_plinkr_filename("select_samples.fam")
#'   ),
#'   sample_selector = create_test_random_sample_selector()
#' )
...
create_select_samples_args <- function(
  ...
  if (plinkr::is_single_sample_selector(sample_selector)) {
    ...
    testthat::expect_true(plinkr::is_random_sample_selector(sample_selector))
    return(
      plinkr::create_select_random_sample_args(
        plink_bin_filenames = plink_bin_filenames,
        random_sample_selector = sample_selector,
        base_output_filename = base_output_filename,
        plink_options = plink_options
      )
    )
